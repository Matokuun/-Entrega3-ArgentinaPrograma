...
public class App 
{
	public static void main(String[] args) {
		...
		Path archivo =Paths.get(args[0]);//archivo csv con resultados
		Path archivo2 =Paths.get(args[1]);//archivo csv con configuracion= conexion a BD, puntaje x partido bien, puntos extra
		...
		ArrayList<ArrayList<Ronda>> fases= new ArrayList<ArrayList<Ronda>>();//es un arrayList con las Rondas de cada fase
		...
		fases.stream().forEach(rondasDeFases -> rondasDeFases.stream().forEach(ronda -> System.out.println("Fase: "+ronda.getFase()+" "+ronda.toString()+"\n")));
		...
		int intFaseActualTabla=0;
		Scanner s= new Scanner(System.in);
		System.out.println("Elija un equipo para ver si los participantes acertaron sus resultados en una fase (Si alguien acierta gana puntos extra)");
		...
		try {
            ...
            String sql= "SELECT id, Apellido, Nombre, Puntos, EquipoLocal, EquipoVisitante, EquipoGanador FROM pronosticos.participantes";
            ...
            while (resultadoSQL.next()) {
                ...
                if(i==0) { //si o si hay que crear el primer participante
                	...
                	if(!((Apellido+","+Nombre).equals(nombreActual))) { //si cambia el nombre y apellido, es porque es otra persona, asi que tengo que ver sus aciertos y demas
    			...
    			if(pronostico.puntos()==1) {
    				...
    				if((equipo1.getNombre().equals(EquipoFase))||(equipo2.getNombre().equals(EquipoFase))) {//si acerto al resultado, suma
	...
	static boolean acertoTodo(ArrayList<Boolean> booleans) { //esta funcion es muy utilizable si añado mas reglas
		boolean acerto= true;
	...
	//en caso de querer crear mas reglas para los puntos extra, se debe crear un nuevo arraylist de booleans,
	// un booleano de seguimiento (para los cambios), y la variable actual para ver cuando se hace el analisis
